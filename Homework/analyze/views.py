...
from datas import oneyear_cate  as oneyear_cate
...
def category(request):
    if 'cate_one' in request.GET:
        cate_name = request.GET['cate_one']
        cate_name = cate_name + '>' + request.GET['cate_two'] if request.GET.has_key('cate_two') else cate_name
        cate_name = cate_name + '>' + request.GET['cate_three'] if request.GET.has_key('cate_three') else cate_name


        # stars = '["1 Stars",44],["2 Stars",23],["3 Stars",22],["4 Stars",21],["5 Stars",13]]'
        stars = '["1 Stars",%d],["2 Stars",%d],["3 Stars",%d],["4 Stars",%d],["5 Stars",%d]]' % oneyear_cate.star(cate_name)



        # comments = '{"2013-02-10":11,"2013-02-11":6,"2013-02-12":3,"2013-02-13":2,"2013-02-14":5,"2013-02-15":3,"2013-02-16":8,"2013-02-17":6,"2013-02-18":6}'



        # prices = '{"2013-02-10":11,"2013-02-11":6,"2013-02-12":3,"2013-02-13":2,"2013-02-14":5,"2013-02-15":3}'
        prices = ['"%s":%.2f' % (x, y) for x, y in datas.oneyear_cate.prize(cate_name) if y is not None]
        comments = ['"%s":%d' % (x, y) for x, y in datas.oneyear_cate.comment(cate_name) ]
        prices = '{' + ','.join(prices) + '}'
        comments = '{' + ','.join(comments) + '}'


        return render_to_response("catogary_show.html",locals())


    return render_to_response("catogary.html")
...
def product_info(request):
    if 'ASIN' in request.GET:
        ...
        if asin != "":
            ...
            prices = ['"%s":%.2f' % (x, y) for x, y in datas.oneyear.prize(asin) if y is not None]
